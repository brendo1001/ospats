function [strat0,StratBest,ObarBest,ObarH1,d2,Sd2] = OS_10a(x,y,z_pred,s2,R2,ra,H,ncy,start,maxrun,d20)
...
% d20: distance ^2 matrix (if used from a previous run)
...
if start == 1  % use Cum-sqrt-f
    nclass = 100;
    if(H>30), nclass=300; end;
    [xm, strat0] = cumsqf(z_pred, nclass, H);
else  % use kmeans 
...
if(n>1000),
    [Z2,S2,Lag]=dist2ab(x,y,z_pred,s2);
elseif(n<100),
    % for large n, use loops
    for i=1:n
    for j=1:n
        Z2(i,j)=(z_pred(i)-z_pred(j)).^2;
        S2(i,j)=s2(i)+s2(j);
        Lag(i,j)=sqrt((x(i)-x(j)).^2+(y(i)-y(j)).^2);        
    end
    end
else
    % else use verctorised calculation
    xy = [x,y];
    x1 = permute(xy, [ 1 3 2 ]);
    y1 = permute(xy, [ 3 1 2 ]);
...
save d20os d20; % save d2 initial

clear Z2 S2 Lag Cov x y z_pred s2 d20;
